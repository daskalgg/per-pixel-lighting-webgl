...
function handleLoadedTexture(texture) {
  ...
  gl.texImage2D(
    ...
    gl.UNSIGNED_BYTE,
    texture.image
  ); // ** loads the image data to our texture.
...
function setupAttributes() {
  ...
  shaderProgram.textureCoordAttribute = gl.getAttribLocation(
    shaderProgram,
    "aTextureCoord"
  ); // ** We get the attribute index for the texture coords attribute.
  ...
  shaderProgram.specularColor = gl.getUniformLocation(
    shaderProgram,
    "uSpecularColor"
  );
...
function initBuffers() {
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(sphere.texCoords),
    gl.STATIC_DRAW
  );
...
function drawScene() {
  ...
  gl.vertexAttribPointer(
    ...
    sphereBuffers.normals.itemSize,
    gl.FLOAT,
    false,
    0,
    0
  ...
  gl.vertexAttribPointer(
    ...
    sphereBuffers.texCoords.itemSize,
    gl.FLOAT,
    false,
    0,
    0
  ...
  gl.uniform3fv(shaderProgram.lightingDirectionUniform, normalize([1, 0, -1]));
  ...
  gl.drawElements(
    gl.TRIANGLES,
    sphereBuffers.indices.numItems,
    gl.UNSIGNED_SHORT,
    0
  );
}
